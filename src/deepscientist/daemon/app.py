...
class DaemonApp:
    ...
    def __init__(
        ...
        *,
        ...
        self.pi_runner = PiRunner(
            home=home,
            repo_root=self.repo_root,
            binary=self.runners_config.get("pi", {}).get("binary", "pi"),
            logger=self.logger,
            prompt_builder=self.prompt_builder,
            artifact_service=self.artifact_service,
        )
        ...
        self.runners = {
            ...
            for name in ("codex", "claude", "kimi", "opencode", "pi")
        }
    ...
    def _continuation_anchor_for(snapshot: dict) -> str:
        available_stage_skills = current_standard_skills(repo_root(), snapshot=snapshot)
